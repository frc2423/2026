...
public class ShooterSubsystem extends SubsystemBase {
    ...
    public ShooterSubsystem(int motorId, boolean isInverted) {
        ...
        // config.closedLoop.p(0.005).i(0).d(0).outputRange(0, 1);
        config.closedLoop.p(0.005).i(0).d(0);

        // if (!isInverted) {
        //     config.closedLoop.p(0.005).i(0).d(0).outputRange(0, 1);
        // } else {
        //     config.closedLoop.p(0.005).i(0).d(0).outputRange(-1, 0);
    ...
    public void periodic() {
        ...
        if (shooterSetpoint == 0) {
            ...
            double velocity = 0;
            boolean isAboveSetpoint = velocity > (shooterSetpoint + 100);
            boolean isBelowSetpoint = velocity < (shooterSetpoint - 100);
